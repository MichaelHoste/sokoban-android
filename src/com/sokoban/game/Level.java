...
public class Level {
	...
	private ArrayList<Character> original_grid; // Original grid of the level
  ...
  public Level(String line, int width, int height, String pack_name, String level_name, String copyright) {
	  ...
	  this.original_grid = new ArrayList<Character>();
	  this.grid          = new ArrayList<Character>();

	  for(int i=0;i<height*width;i++) {
	    char c = line.charAt(i);
	    original_grid.add(Character.valueOf(c));
	  }
	  
    grid = (ArrayList<Character>) original_grid.clone();
  ...
  //         1 if normal move.
  //         2 if box move.
  public int delete_last_move(Path path) {
    int action = 1;
    int maj = 0;
    int m = pusher_pos_m;
    int n = pusher_pos_n;
    char direction = path.get_last_move();
    int state = 1;
  ...
  // @return true if all boxes are in their goals, false if not
  boolean is_solution_path(Path path) {
    for(int i=0; i<path.moves.size(); i++)
      move(path.moves.get(i));
